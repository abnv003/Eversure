        ...
        {renderImage.map((item, key) => {
          ...
          return (
            <div 
              ...
              ref={el => sectionRefs.current[key] = el}
              data-index={key}
              className="relative px-4 md:px-60 pt-20 mb-20 overflow-hidden"
            >
              {/* Mobile Layout */}
              <div className="block md:hidden">
                {/* Image */}
              ...
              <div className="hidden md:block overflow-hidden">
                {/* Image Container - Full Width Background */}
                <div 
                  className={`absolute inset-0 flex items-center transition-all duration-1000 ease-out overflow-hidden ${
                    isCurrentVisible 
                      ? 'opacity-100 translate-x-0' 
                ...
                {/* Text Content - Overlapping Image */}
                <div className={`relative z-20 flex ${isImageLeft ? 'justify-end' : 'justify-start'} overflow-hidden`}>
                  <div className={`w-3/5 ${isImageLeft ? 'ml-auto' : 'mr-auto'}`}>
                    <div 
                      className={`bg-white p-12 transition-all duration-1000 ease-out delay-300 ${
                        isCurrentVisible 
                        ...
                        {item.content.map((contentItem, index) => (
                          <div 
                            ...
                            className={`flex items-start gap-4 transition-all duration-1000 ease-out ${
                              isCurrentVisible 
                                ? 'opacity-100 translate-x-0' 
                                : `opacity-0 ${isImageLeft ? 'translate-x-6' : '-translate-x-6'}`
                            }`}
                            style={{
                              transitionDelay: `${800 + (index * 150)}ms`
                            }}
